...
Version 2.3 adds support for modern JavaScript (ES2015 through ES2023+) via [Babel] preprocessing.

The original 2.0 release uses [Esprima] 2.5 which supports ES5 and partial ES6 (ES2015).
Version 2.3 adds [Babel] as an optional preprocessing step to transpile modern JavaScript
syntax down to a version that the existing js2coffee transforms can handle.
...
**Auto-detection:** Babel preprocessing is automatically enabled when modern syntax
is detected. Can be controlled via `build()` options:
...
A powerful use case emerges from this architecture: **converting TypeScript to CoffeeScript**.
...
The result is idiomatic CoffeeScript.

See **[CHANGELOG.md](../CHANGELOG.md)** for full details on version 2.3.

## About js2coffee 2.0

The 2.0 release is a complete rewrite featuring a better parser ([Esprima]).
